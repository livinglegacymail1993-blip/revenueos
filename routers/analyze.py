...
from core.session_store import COOKIE_NAME, get_creds_from_cookie_value
...
@analyze_router.post("/live")
def analyze_live(request: Request):
    """
    Run analysis using the session's Stripe OAuth token (no pasted key).
    Requires prior Connect OAuth; returns 401 if not connected.
    """
    cookie_value = request.cookies.get(COOKIE_NAME)
    creds = get_creds_from_cookie_value(cookie_value)
    if not creds:
        raise HTTPException(status_code=401, detail="Not connected. Connect Stripe first.")
    stripe_api_key = creds["stripe_access_token"]
